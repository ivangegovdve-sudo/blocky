...
func NewListCache(ctx context.Context,
	...
	err := cfg.StartPeriodicRefresh(ctx, c.refresh, func(err error) {
		logger().Error(fmt.Sprintf("could not init %s", t), log.AttrError(err))
	})
	if err != nil {
	...
	producers.GoConsume(func(ctx context.Context, ch <-chan string) error {
		for host := range ch {
			if groupFactory.AddEntry(host) {
				...
				logger().With(slog.String("host", host)).Warn("no list cache was able to use host")
			}
		}
	...
	for group, sources := range localSources {
		unlimitedGrp.Go(func(ctx context.Context) error {
			if err := b.createCacheForGroup(producersGrp, unlimitedGrp, group, sources); err != nil {
				logger().With(log.AttrError(err), slog.String(logFieldGroup, group)).Debug("disk seed: skipping group with no usable local data")
...
func (b *ListCache) parseFile(ctx context.Context, opener SourceOpener, resultCh chan<- string) error {
	...
	if err != nil {
		logger().Error("cannot open source", log.AttrError(err))
	...
	p.OnErr(func(err error) {
		logger().Warn(fmt.Sprintf("parse error: %s, trying to continue", err))
	})
	...
	if err != nil {
		...
		if !errors.Is(err, context.Canceled) {
			logger().Error("parse error", log.AttrError(err))
		}
