...
func (c *cachingDownloader) DownloadFile(ctx context.Context, link string) (io.ReadCloser, error) {
	...
	if err != nil {
		...
		if !errors.As(err, &statusErr) {
			if cached, openErr := openCached(c.dir, link); openErr == nil {
				logger().Warn("download failed, using cached copy", slog.String("link", link), log.AttrError(err))
	...
	if resp.statusCode == http.StatusNotModified {
		...
		if openErr == nil {
			logger().Debug("source not modified, using cached copy", slog.String("link", link))
...
func (c *cachingDownloader) serveAndStore(link string, resp *downloadResponse) (io.ReadCloser, error) {
	...
	fin.finalize = func() {
		...
		if err := os.Rename(tmp.Name(), finalPath); err != nil {
			logger().Warn(fmt.Sprintf("cannot finalize cache file %s", finalPath), log.AttrError(err))
			_ = os.Remove(tmp.Name())
