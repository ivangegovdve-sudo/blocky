...
func NewRedisExpiringCache[T any](
	...
	if err := c.loadFromRedis(ctx); err != nil {
		c.logger.Warn("startup Redis scan failed – starting with empty local cache", log.AttrError(err))
	}
...
func (c *RedisExpiringCache[T]) loadFromRedis(ctx context.Context) error {
	return c.scanKeys(ctx, func(ctx context.Context, keys []string) error {
		if err := c.loadKeysViaPipeline(ctx, keys); err != nil {
			c.logger.Warn("pipeline load failed for SCAN page", log.AttrError(err))
		}
...
func (c *RedisExpiringCache[T]) loadKeysViaPipeline(ctx context.Context, keys []string) error {
	...
	for i, rk := range keys {
		...
		if err != nil {
			c.logger.Warn("failed to decode Redis entry on startup", log.AttrError(err))
...
func (c *RedisExpiringCache[T]) flushBatch(ctx context.Context, batch []sendBufferEntry[T]) {
	...
	for _, e := range batch {
		...
		if err != nil {
			c.logger.Warn("failed to encode cache entry for Redis", log.AttrError(err))
	...
	if _, err := pipe.Exec(ctx); err != nil && ctx.Err() == nil {
		c.logger.Warn("Redis pipeline exec failed", log.AttrError(err))
	}
	...
	if err != nil {
		c.logger.Warn("failed to marshal sync message", log.AttrError(err))

		return
	}

	if err := c.client.Publish(ctx, c.opts.Channel, payload).Err(); err != nil && ctx.Err() == nil {
		c.logger.Warn("Redis PUBLISH failed", log.AttrError(err))
	}
}
...
func (c *RedisExpiringCache[T]) handleSyncMessage(ctx context.Context, payload []byte) {
	...
	if err := json.Unmarshal(payload, &msg); err != nil {
		c.logger.Warn("failed to decode sync message", log.AttrError(err))
	...
	if _, err := pipe.Exec(ctx); err != nil {
		c.logger.Warn("failed to fetch TTLs for sync entries", log.AttrError(err))
	...
	for i, e := range msg.Entries {
		...
		if err != nil {
			c.logger.Warn("failed to decode sync entry", log.AttrError(err))
