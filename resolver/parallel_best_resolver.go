...
func (r *ParallelBestResolver) Resolve(ctx context.Context, request *model.Request) (*model.Response, error) {
	...
	if len(allResolvers) == 1 {
		...
		logger.Debug("delegating to resolver", slog.Any("resolver", resolver.resolver))
...
outer:
	...
	if err != nil {
		log.FromCtx(ctx).Error("can't choose random weighted resolver, falling back to uniform random", log.AttrError(err))
