...
func (r *ClientNamesResolver) resolveClientNames(ctx context.Context, ip net.IP) (result []string) {
	...
	if names := r.lookupLocalReverse(ip); len(names) > 0 {
		...
		logger.Debug("resolved client name(s) from local reverse lookup",
	...
	if err != nil {
		logger.Error("can't resolve client name", log.AttrError(err))
	...
	logger.Debug("resolved client name(s) from external resolver",
		slog.String("client_names", strings.Join(result, "; ")))
